...
int main(int argc, char *argv[]){
        ...
        struct prinfo* nulltask = NULL;
	//For testing with current process
	mytaskinfo->pid = getpid();
	//for testing with pid=1
        //mytaskinfo->pid = 1;//init
	
	printf("\nDiving to kernel level\n\n");
        int ret = syscall(_prinfo_,mytaskinfo);
	//for testing NULL pointer
	//int ret = syscall(_prinfo_,nulltask);
        
	printf("\nRising to user level\n\n");
        
	if(ret!=0){
		printf(" Sys call failed with error code %d",ret);
